...
stdenv.mkDerivation {
  ...
  postFixup =
    ...
    # that case.
    + optionalString (targetPlatform ? gcc.arch || targetPlatform.parsed.cpu ? arch) ''
      echo "-march=${targetPlatform.gcc.arch or targetPlatform.parsed.cpu.arch}" >> $out/nix-support/cc-cflags-before
    ''
    ...
    + optionalString (targetPlatform ? gcc.float-abi || targetPlatform.parsed.abi ? float) ''
      echo "-mfloat-abi=${targetPlatform.gcc.float-abi or targetPlatform.parsed.abi.float}" >> $out/nix-support/cc-cflags-before
    ''
