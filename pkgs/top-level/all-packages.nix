...
in
{
  ...
  makeWrapper = makeSetupHook { deps = [ dieHook ]; substitutions = { shell = targetPackages.runtimeShell or runtimeShell; }; }
  ...
  wrapGAppsHook = makeSetupHook {
    deps = lib.optional (!stdenv.isDarwin) targetPackages.dconf.lib ++ [ targetPackages.gtk3 targetPackages.librsvg makeWrapper ];
