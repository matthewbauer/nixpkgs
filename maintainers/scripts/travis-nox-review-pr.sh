...
while test -n "$1"; do
    ...
    case $1 in
        ...
        nixpkgs-verify)
            ...
            nix-env --file $TRAVIS_BUILD_DIR --query \
                    --available --json > /dev/null
            ;;

        nixos-options)
            echo "=== Checking NixOS options"

            nix-build $TRAVIS_BUILD_DIR/nixos/release.nix \
                      --attr options --show-trace
            ;;
        ...
        nixos-manual)
            ...
            nix-build $TRAVIS_BUILD_DIR/nixos/release.nix \
                      --attr manual --show-trace
            ;;

        nixpkgs-manual)
            echo "=== Checking nixpkgs manuals"

            nix-build $TRAVIS_BUILD_DIR/pkgs/top-level/release.nix \
                      --attr manual --show-trace
            ;;
        ...
        nixpkgs-tarball)
            ...
            nix-build $TRAVIS_BUILD_DIR/pkgs/top-level/release.nix \
                      --attr tarball --show-trace
            ;;

        nixpkgs-unstable)
            echo "=== Checking nixpkgs unstable job"

            nix-instantiate $TRAVIS_BUILD_DIR/pkgs/top-level/release.nix \
                            --attr unstable --show-trace
            ;;

        nixpkgs-lint)
            echo "=== Checking nixpkgs lint"

            nix-shell --packages nixpkgs-lint \
